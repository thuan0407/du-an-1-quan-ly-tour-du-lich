<div class="schedule-container">
  <h2>✏️ Sửa Yêu cầu đặc biệt</h2>

  <form id="editSpecialForm" method="POST">
    <div class="form-group">
      <label for="date">Ngày:</label>
      <input type="date" id="date" name="date" value="<?= htmlspecialchars($request['date']) ?>" required>
    </div>

    <div class="form-group">
      <label for="content">Nội dung:</label>
      <textarea id="content" name="content" rows="4" required><?= htmlspecialchars($request['content']) ?></textarea>
      <small id="contentError" style="color:red; display:none;"></small>
    </div>

    <input type="hidden" name="status" value="1">

    <button type="submit" name="submit_special_request" class="btn btn-primary">Lưu thay đổi</button>
    <a href="?action=special_request_index&id_book_tour=<?= $request['id_book_tour'] ?>" class="btn btn-secondary">← Quay lại</a>
  </form>
</div>
...
<style>
/* CSS giống view add, có thể tái sử dụng */
.schedule-container {
    max-width: 700px;
    margin: 30px auto;
    background: #fff;
